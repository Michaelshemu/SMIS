...
for($i =2; $i<=count($sheetData);$i++) {
   $one=$sheetData[$i]['A']; $two=$sheetData[$i]['B']; $three=$sheetData[$i]['C'];$four=$sheetData[$i]['D'];
	  $five= $sheetData[$i]['E']; $six= $sheetData[$i]['F'];$seven= $sheetData[$i]['G'];$eight= $sheetData[$i]['H'];$nine= $sheetData[$i]['I'];$ten= $sheetData[$i]['J'];$eleven= $sheetData[$i]['K'];
     
	 
	 
	 $ise=md5($four);
   $query1 = "insert into users(username,password,status,user_level,info_id) VALUES ('$six','$ise','1','student','$one')";
    mysql_query($query1) or die(mysql_error());
	
	$query2="insert into user_info (id,fname,mname,lname,gender) values('$one','$two','$three','$four','$five')";
    mysql_query($query2) or die(mysql_error());
	
	$query3="insert into parent (fname,mname,lname,username,password,student_id,status,user_level) values('$seven','$eight','$nine','$ten',md5('$nine'),'$one','1','parent')";
